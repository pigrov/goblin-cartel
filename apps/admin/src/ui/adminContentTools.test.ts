...
const baseContent = {
  ...
  blockTypes: [{ id: "draft_block_03", nameKey: "block.existing.name" }],
  mineTemplates: [],
  goblins: [],
  localization: {
    ru: {
...
describe("admin draft templates", () => {
  it("creates a new block type and selects it for editing", () => {
    ...
    expect(result.entityId).toBe("draft_block_02");
    expect(createdBlock).toMatchObject({
      baseHp: 80,
      id: "draft_block_02",
      specialBehavior: "none"
    });
    expect(result.content.localization?.ru?.["block.draft_block_02.name"]).toBe("Новый блок");
  });
